...
class dataQ():
	...
	def __init__(self,hud):
		############File_argumensts####################################
		self.date='03_07_19'
		self.test='2'                      # indication of the experiment number
	...
	def data_manage(self,actor):
		...
		time=self.HUD.simulation_time-self.start_time
		...
		self.actors[_id]=np.vstack((self.actors[_id],[time,_id,hero,location.x,location.y,location.y,rotation.pitch,rotation.yaw,rotation.roll,
			velocity.x,velocity.y,velocity.z,accl.x,accl.y,accl.z,Wvel.x,Wvel.y,Wvel.z,control.throttle,control.steer,
			control.brake]))
		pass
	def data_input(self,actor_list):
		'''
	...
	def data_save(self):
		...
		'''
		heading="Time,VehicleID,EGO,X,Y,Z,Roll,Pitch,Yaw,Vx,Vy,Vz,Ax,Ay,Az,Wx,Wy,Wz,Throttle,Steer,Brake"
		self.data=np.empty((0,self.attr_num))
		for _id in self.actors:
			self.data=np.vstack((self.data,self.actors[_id]))

		file_name=os.path.join(self.data_dir,self.date+"_"+self.test+"_"+".csv")
